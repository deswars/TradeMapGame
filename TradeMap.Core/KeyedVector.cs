...
namespace TradeMap.Core
{
    ...
    {
        public Dictionary<TIndex, double> Dict { get; private set; }
        ...
        public KeyedVector(IEnumerable<TIndex> indexes)
        {
            Dict = new();
        ...
        public KeyedVector<TIndex> FilterSmaller(List<TIndex> keys)
        {
            KeyedVector<TIndex> result = new();
            foreach (var key in keys)
            {
